//1. Დაბეჭდეთ რიცხვები 2 დან 8 მდე
//for (let i=2; i<8; i++) {console.log (i)}

// 2. 5 დან 35 - ის ჩათვლით დაბეჭდეთ ყოველი მეოთხე რიცხვი (უნდა დაიბეჭდოს 5,9,13…)
//for (let i=5; i<36; i+=4)
   // {console.log(i)}

// 3. დაბეჭდეთ 3 დან 8 მდე რიცხვების ნამრავლი
//let y=1
// for (let i=3; i<8; i++) (y *=i)
 //console.log (y)

 
// 4. Გამოაცხადეთ ობიექტი person რომელსაც აქვს firstName, lastName, age
//  და დაბეჭდეთ person-სრული სახელი ( სახელი და გვარი)
// let person={
//    fisrtName:"Tako",
//     lastName:"Tsikarishvili",
//     age:34
// } 
// console.log(person.fisrtName, person.lastName)


// 5. დაბეჭდეთ მეოთხე დავალებაში გამოცხადებული პერსონის თითოეული property value 
// let person={
//     fisrtName:"Tako",
//      lastName:"Tsikarishvili",
//      age:34
//  } 
//   console.log(person)
  //ან 
  //console.log(person.fisrtName, person.lastName, person.age)
  //ან
//for (const personkey in person) {console.log(person[personkey])}


// 6. დაწერეთ მასივი fruits ელემენტებით ["Apple", "Banana", "Orange"]; 
// დაბეჭდეთ მასივის თითოეული წევრი.

//let fruits=["Apple", "Banana", "Orange"]
//for (let i=0; i<fruits.length; i++)
//{console.log (fruits [i])}

//ან 
//for (const element of fruits) {
  //  console.log (element)}


// 7. დაამატეთ fruits მასივის თავში Grapes და ბოლოში  Pineapples დაბეჭდეთ fruits
// let fruits=["Apple", "Banana", "Orange"]
// fruits.unshift("Grapes")
// fruits.push("Pienapples")
// for (const element of fruits) {
//     console.log (element)
// }


 //8.დაბეჭდეთ 1 დან 34 მდე რიცხვების ჯამი
 //let x=0
//for (let i=1; i<34; i++) {x += i}
//console.log (x)
